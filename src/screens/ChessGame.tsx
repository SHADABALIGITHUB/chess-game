...
import { ChevronLeft, ChevronRight, RotateCcw, Play, Crown, Lightbulb } from 'lucide-react';
...
const Piece: React.FC<PieceProps> = ({ piece, isWhite }) => {
  return (
    <div className={`text-4xl ${isWhite ? 'text-white' : 'text-black'} drop-shadow-md transition-transform hover:scale-110`}>
      {pieces[piece]}
    </div>
  );
};
...
const Square: React.FC<SquareProps> = ({ piece, isBlack, onClick, isSelected, isSuggested }) => {
  ...
  return (
    <div 
      className={`w-16 h-16 flex items-center justify-center ${bgColor} ${selectedClass} ${suggestedClass} cursor-pointer transition-all duration-300 ease-in-out hover:opacity-80`}
      onClick={onClick}
    >
...
const Chessboard: React.FC<ChessboardProps> = ({ board, onSquareClick, selectedSquare, suggestedMove }) => {
  return (
      ...
      {board.flatMap((row, i) =>
        row.map((piece, j) => (
          <Square
            ...
            isSelected={selectedSquare !== null && selectedSquare[0] === i && selectedSquare[1] === j}
            isSuggested={suggestedMove !== null && 
...
const Timer: React.FC<TimerProps> = ({ time, isActive }) => {
  ...
  return (
    <div className={`text-3xl font-mono font-bold ${isActive ? 'text-emerald-500' : 'text-gray-500'} transition-colors duration-300`}>
...
const ChessGame: React.FC = () => {
  ...
  const isValidMove = (fromRow: number, fromCol: number, toRow: number, toCol: number): boolean => {
    ...
    if (toPiece && toPiece.toLowerCase() === toPiece && piece.toLowerCase() === piece) return false;
    if (toPiece && toPiece.toUpperCase() === toPiece && piece.toUpperCase() === piece) return false;
    ...
    switch (piece.toLowerCase()) {
      case 'p':
        if (fromCol === toCol && !toPiece) {
          ...
          if ((fromRow === 1 && piece === 'p') || (fromRow === 6 && piece === 'P')) {
            if (toRow === fromRow + 2 * direction && !board[fromRow + direction][fromCol]) return true;
          }
        }
        if (Math.abs(fromCol - toCol) === 1 && toRow === fromRow + direction) {
          if (toPiece) return true;
          if ((fromRow === 3 && piece === 'P') || (fromRow === 4 && piece === 'p')) {
            const lastMove = moveHistory[moveHistory.length - 1];
            if (lastMove && lastMove[toRow - direction][toCol] === (piece === 'P' ? 'p' : 'P') &&
                lastMove[fromRow][toCol] === null && board[fromRow][toCol] === (piece === 'P' ? 'p' : 'P')) {
              return true;
            }
          }
        }
      ...
      case 'k':
        ...
        if (Math.abs(fromCol - toCol) === 2 && fromRow === toRow) {
          ...
          if (rookPiece === (piece === 'K' ? 'R' : 'r') && !hasKingMoved(turn) && !hasRookMoved(turn, rookCol)) {
  ...
  const isSquareUnderAttack = (row: number, col: number, defendingColor: Turn): boolean => {
    for (let i = 0; i < 8; i++) {
      for (let j = 0; j < 8; j++) {
        const piece = board[i][j];
        if (piece && (defendingColor === 'white' ? piece === piece.toLowerCase() : piece === piece.toUpperCase())) {
  ...
  const handleSquareClick = (i: number, j: number) => {
    ...
    if (selectedSquare) {
      ...
      if (i !== si || j !== sj) {
        if (isValidMove(si, sj, i, j)) {
          ...
          if (newBoard[i][j]?.toLowerCase() === 'p') {
            ...
            if ((turn === 'white' && i === 0) || (turn === 'black' && i === 7)) {
              newBoard[i][j] = turn === 'white' ? 'Q' : 'q'; // Auto-promote to queen for simplicity
            }
          
            if (Math.abs(si - i) === 1 && Math.abs(sj - j) === 1 && !capturedPiece) {
              newBoard[si][j] = null; // Remove the captured pawn
            }
          } else if (newBoard[i][j]?.toLowerCase() === 'k' && Math.abs(sj - j) === 2) {
      ...
      const piece = board[i][j];
      if (piece && ((turn === 'white' && piece.toUpperCase() === piece) ||
  ...
  const suggestMove = () => {
    const currentPlayerPieces = turn === 'white' ? ['P', 'R', 'N', 'B', 'Q', 'K'] : ['p', 'r', 'n', 'b', 'q', 'k'];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br rounded-lg from-emerald-800 to-emerald-600 flex items-center justify-center p-4">
      <Card className="w-full max-w-4xl  bg-white shadow-2xl">
        <CardHeader className="bg-emerald-700">
          <CardTitle className="text-4xl font-bold text-center text-white">Chess Game</CardTitle>
        </CardHeader>
        <CardContent className="flex flex-col items-center space-y-6 p-8">
          <div className="text-2xl font-semibold bg-emerald-100 px-6 py-3 rounded-full text-emerald-800">
            {isGameStarted ? `Turn: ${turn === 'white' ? 'White' : 'Black'}` : 'Press Start to begin'}
          </div>
          <div className="flex items-center space-x-8 bg-emerald-100 p-4 rounded-lg">
            <Timer time={whiteTime} isActive={isGameStarted && turn === 'white'} />
            <Crown className={`w-8 h-8 ${turn === 'white' ? 'text-yellow-500' : 'text-gray-400'}`} />
            <Timer time={blackTime} isActive={isGameStarted && turn === 'black'} />
          </div>
          <Chessboard 
            board={board} 
        ...
        <CardFooter className="flex justify-center space-x-4 bg-emerald-100 p-6">
          {!isGameStarted && (
            <Button onClick={handleStartGame} className="bg-emerald-600 hover:bg-emerald-700 text-white">
              <Play className="mr-2 h-5 w-5" /> Start Game
            </Button>
          )}
          <Button onClick={handleUndo} disabled={!isGameStarted || currentMove === 0} className="bg-blue-600 hover:bg-blue-700 text-white">
            <ChevronLeft className="mr-2 h-5 w-5" /> Undo
          </Button>
          <Button onClick={handleRedo} disabled={!isGameStarted || currentMove === moveHistory.length - 1} className="bg-blue-600 hover:bg-blue-700 text-white">
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button onClick={suggestMove} disabled={!isGameStarted || isGameOver} className="bg-yellow-500 hover:bg-yellow-600 text-white">
      ...
      <Dialog open={showWinModal} onOpenChange={setShowWinModal}>
        <DialogContent className="bg-white p-6 rounded-lg shadow-xl">
          ...
          <div className="mt-6 flex justify-center">
            <Button onClick={handleReset} className="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2 rounded-full text-lg">
              Play Again
            </Button>
          </div>
        </DialogContent>
